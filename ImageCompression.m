...
%Alternate: [Y,Cb,Cr] = imsplit(ImageYCbCr);
...
%Zero Padding the image to get integer number of 8x8 Blocks:
%For Luma(Y) Image:
if remainder(height(Y),8)~=0
    Y=[Y;zeros(8-remainder(height(Y),8),width(Y))];
end
if remainder(width(Y),8)~=0
    Y=[Y zeros(height(Y),8-remainder(width(Y),8))];
end
...
dct = @(blockStruct) myDCT2(blockStruct.data);        %Defining Anonymous Function Handle
...
Y_quant = @(blockStruct) round((blockStruct.data)./Qy);      %Defining Anonymous Function Handle
C_quant = @(blockStruct) round((blockStruct.data)./Qc);      %Defining Anonymous Function Handle

Y_dct_quant = blockproc(Y_dct, [8 8], Y_quant);
Cb_down_dct_quant = blockproc(Cb_down_dct, [8 8], C_quant);
Cr_down_dct_quant = blockproc(Cr_down_dct, [8 8], C_quant);
...
Y_dequant = @(blockStruct) round((blockStruct.data).*Qy);      %Defining Anonymous Function Handle
C_dequant = @(blockStruct) round((blockStruct.data).*Qc);      %Defining Anonymous Function Handle

Y_dct_dequant = blockproc(Y_dct_quant, [8 8], Y_dequant);
Cb_down_dct_dequant = blockproc(Cb_down_dct_quant, [8 8], C_dequant);
Cr_down_dct_dequant = blockproc(Cr_down_dct_quant, [8 8], C_dequant);
...
idct = @(blockStruct) myIDCT2(blockStruct.data);        %Defining Anonymous Function Handle

Y_idct = blockproc(Y_dct_dequant, [8 8], idct);
Cb_down_idct = blockproc(Cb_down_dct_dequant, [8 8], idct);
Cr_down_idct = blockproc(Cr_down_dct_dequant, [8 8], idct);
...
%Re-shifting the blocks to original center(128):
...
RGB_reconstruct = ycbcr2rgb(YCbCr_reconstruct);
...
imwrite(RGB_reconstruct,'E:\Engineering\Projects\Image Compression using DCT\Image Compression\Images\Compressed Images\Test.jpg');
